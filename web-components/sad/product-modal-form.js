import productModalFormStyle from '/modal/product-modal-form-style.js';
...
const htmlText = /* html */ ` 
  <form class="modal-form">
    <ul class="modal-ul">
      ...
      <li class="modal-li">
        ...
        <div class="select select-category-container">
          <select id="options-root" class="select-category">
            <slot></slot>
          </select>
        </div>
      </li>  
    ...
    <button id="btn-text-root" class="modal-button" type="submit"></button>
  </form>
`;

const modalTemplate = document.createElement('template');
modalTemplate.innerHTML = `
  ${productModalFormStyle}
  ${htmlText}
`;

export default class ProductModalForm extends HTMLElement {
  ...
  constructor() {
    ...
    // Roots
    this.optionsRoot = this.shadowRoot.getElementById('options-root');
    this.btnText = this.shadowRoot.getElementById('btn-text-root');
  ...
  async connectedCallback() {
    // category component와 중복 호출 해결 필요
    const categories = await Api.get('/category/list');
    this.createCategoryOptions(categories);
  }
  ...
  createCategoryOptions(categories) {
    ...
    this.optionsRoot.insertAdjacentHTML('afterbegin', categoryOptionsHTML);
  }
  ...
  render() {
    ...
    if (this.modalType === `${Modal.getType().addProductModal}`) {
      this.btnText.innerHTML = '등록하기';
    }

    if (this.modalType === `${Modal.getType().editProductModal}`) {
      this.btnText.innerHTML = '수정하기';
    }
  }
}
